...
if (!isset($_SESSION[$update_key])) {
    ...
    try {
        ...
        if ($affected_rows > 0) {
            $success_message = $affected_rows . " pengumuman dinonaktifkan karena sudah melewati tanggal_berakhir.";
            error_log(date('Y-m-d H:i:s') . " - " . $success_message);
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($judul) || empty($isi_pengumuman) || empty($tanggal_mulai)) {
        ...
        if (isset($_POST['id_pengumuman']) && !empty($_POST['id_pengumuman'])) {
            ...
            $stmt->bind_param("ssssss", $judul, $isi_pengumuman, $tanggal_mulai, $tanggal_berakhir, $status_aktif, $id_pengumuman);
            ...
            $stmt = $conn->prepare($query);
            $stmt->bind_param("sssssss", $id_pengumuman, $judul, $isi_pengumuman, $tanggal_mulai, $tanggal_berakhir, $status_aktif, $dibuat_oleh);
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare($query);
    $stmt->bind_param("s", $id_pengumuman);
